package com.prestamosblockchain.security;
...
/**
 * Filtro que capturará las petición antes de ser procesados por los servicios
 * para validar la firma del objeto JWT en la petición.
 * 
 * @author Juan Mariño
 *
 */
public class OpenIdConnectValidationFilter extends AbstractAuthenticationProcessingFilter {
	...
	public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
		...
		if (autorizationHeader == null || cliendId == null) {
			throw new BadCredentialsException("No se están enviando los datos del X-Authorization y/o client_id");
		}
		if (OpenIdConnectValidationFilter.HEADER_PREFIX.length() > autorizationHeader.length()) {
			throw new BadCredentialsException("Valor para X-Authorization no es válido");
		}
		...
		if (googleTokenId == null) {
			throw new BadCredentialsException("Token Id no es válido");
		}
